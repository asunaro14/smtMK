...
$(document).on("pageinit", function (e) {
    try {
        ...
        if(pageName){
        	App.pageName = pageName; //ページ名をグローバルに保存　ハッシュを無効にしたことで、locationから取得ができなくなったため
        }
    ...
    if (alertPage.size() < 1) {
        ...
        if (conf) {
            ...
            if (conf.onclose) {
                ...
                //$("#entry_01_alert").popup({ afterclose: conf.onclose });  // aftercloseイベントハンドラを設置
            }
        } else {
            $("#" + pageName + "_confirm").show();              // OKボタンを表示
        }
        
        //$("#" + pageName + "_alert").popup("open", { transition: "pop" });
